...
def teacher_add_question_view(request):
    ...
    if request.method=='POST':
        if 'csv_submit' in request.POST:
            ...
            course=QMODEL.Course.objects.get(id=request.POST.get('courseID'))  # Use the Course model from the models app
            ...
            for row in reader:
                question = QMODEL.Question(
                    ...
                    answer=get_answer_choice_value(row['answer'], row),
                    marks=row['marks']
            ...
            if questionForm.is_valid():
                ...
                course=models.Course.objects.get(id=request.POST.get('courseID'))  # Use the Course model from the models app
...
def get_answer_choice_value(answer_text, row):
    for option in ['option1', 'option2', 'option3', 'option4']:
        if answer_text == row[option]:
